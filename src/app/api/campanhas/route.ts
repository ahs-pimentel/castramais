...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await pool.query(`
      SELECT id, nome, cidade, uf, "dataInicio", "dataFim", "dataDescricao", limite, ativa, "createdAt"
      FROM campanhas
      ${whereClause}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { nome, cidade, uf, dataInicio, dataFim, dataDescricao, limite } = body
    ...
    const result = await pool.query(
      `INSERT INTO campanhas (nome, cidade, uf, "dataInicio", "dataFim", "dataDescricao", limite, ativa)
       ...
       RETURNING *`,
      [nome.trim(), cidade.trim(), uf || 'MG', dataInicio || null, dataFim || null, dataDescricao || null, limite || 200]
    )
